...
const BoxForget = () => {
    ...
    const { handleSubmit, control, formState: { errors }, register, reset } = useForm<ForgetFormType>()
    ...
    return (
        <>
            <div className="w-72 mx-auto my-6 p-6 rounded-2xl shadow-card">
                {
                    rooter ? (<Navigate to="/connexion" replace={true} />) : (<FormForget form={{ errors, control, register, handleSubmit, onSubmit, isLoading }} />)
                }
            </div>
        </>
    );
